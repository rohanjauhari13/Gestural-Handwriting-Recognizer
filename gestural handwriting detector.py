...
c=0
#for file in os.listdir("C:\\Users\\admin\\Desktop\\GHD_dataset\\test\\z"):
   
#    thresh=cv2.imread(file)
    #cv2.imshow('file',thresh)
#    thresh=cv2.cvtColor(thresh,cv2.COLOR_BGR2GRAY)
 #   _,thresh=cv2.threshold(thresh,100,255,cv2.THRESH_BINARY)
  #  contours, hierarchy=cv2.findContours(thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
   # x,y,w,h=cv2.boundingRect(contours[1])
    #thresh=thresh[y:y+h,x:x+w]
    #cv2.imshow('thresh',thresh)
    #os.remove(file)
    #cv2.imwrite(str(c)+'.png',thresh)
    #c+=1
    #break
...
training_set = train_datagen.flow_from_directory('GHD_dataset/training',
                                                target_size=(64, 64),
                                                batch_size=64,
                                                class_mode='categorical')

test_set = test_datagen.flow_from_directory('GHD_dataset/test',
                                            target_size=(64, 64),
                                            batch_size=128,  
                                            class_mode='categorical')
...
import pickle
#save
new=pickle.dumps(classifier)
#load
classifier=pickle.loads(new)
########################################################################
########################################################################
#   IP
...
while True:
    ...
    if k==ord('a'):
        ...
        cv2.imwrite('thresh.png',img1)
        thresh=cv2.imread('thresh.png')
        thresh=cv2.cvtColor(thresh,cv2.COLOR_BGR2GRAY)
        _,thresh=cv2.threshold(thresh,100,255,cv2.THRESH_BINARY)
        contours, hierarchy=cv2.findContours(thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
        x,y,w,h=cv2.boundingRect(contours[1])
        cv2.imwrite('test.png',img1[y:y+h,x:x+w+5])
